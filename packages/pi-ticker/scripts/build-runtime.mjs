...
const FORBIDDEN_EAGER_INPUTS = ["src/menu.ts", "src/tui-menu.ts"];
...
export async function buildRuntime({
	...
	const stagingDirectory = await mkdtemp(
		join(dirname(resolvedOutputDirectory), ".pi-ticker-dist-"),
	);
	...
	for (const outputPath of eagerOutputs) {
		...
		for (const imported of output.imports ?? []) {
			if (
				...
				FORBIDDEN_EAGER_EXTERNALS.includes(imported.path)
			) {
				throw new Error(
	...
	for (const runtimePath of runtimeFiles) {
		...
		for (const specifier of generatedRelativeImports(source)) {
			if (!specifier.endsWith(".js")) {
				throw new Error(
					`Generated runtime import has an unsupported extension: ${runtimePath} -> ${specifier}`,
				);
			}
...
function generatedRelativeImports(source) {
	return [
...
async function assertSafeOutputDirectory(outputDirectory) {
	...
	const isTestOutput = firstSegment?.startsWith(".pi-ticker-build-test-");
	if (
		relativeOutput === "" ||
		relativeOutput === ".." ||
