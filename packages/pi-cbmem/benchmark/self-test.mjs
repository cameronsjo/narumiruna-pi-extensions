...
assert.equal(BENCHMARK_ID, "pi-cbmem-retrieval-comparison:v3");
...
const daemonBridge = { transport: "mcp-stdio", daemonBacked: true, serverPid: 123 };
...
const exactScore = scoreTrial({
	...
	toolResults: [{ name: "search_code", text: packet, bridge: daemonBridge }],
	evidencePacket: packet,
});
assert.equal(exactScore.success, true);
assert.equal(exactScore.exactPayload.matched, true);
const unverifiedTransport = scoreTrial({
	arm: "cbmem",
	task: exact,
	responseText,
	toolCalls: [{ name: "search_code", args: exact.exactTool.args }],
...
assert.equal(unverifiedTransport.success, false);
assert.match(unverifiedTransport.errors.join("\n"), /lacked verified daemon-backed mcp-stdio/);
assert.equal(
	scoreTrial({
		arm: "cbmem",
		task: exact,
...
const discoveredAgain = scoreTrial({
	...
	toolResults: [
		{ name: "list_projects", text: "{}", bridge: daemonBridge },
		{ name: "search_code", text: "beta", bridge: daemonBridge },
	],
	project: "project-name",
});
...
assert.equal(
	scoreTrial({
		...
		toolResults: [{ name: "search_code", text: "beta", bridge: daemonBridge }],
		project: "project-name",
	}).success,
	false,
);
...
function handle(command) {
	if (command.type === "set_auto_compaction" && process.argv.includes("slow-start")) {
		...
		send({ type: "tool_execution_end", toolCallId: "call-1", toolName: "search_code", result: { content: [{ type: "text", text: ${JSON.stringify(packet)} }], details: { bridge: { transport: "mcp-stdio", daemonBacked: true, serverPid: 123 } } }, isError: false });
