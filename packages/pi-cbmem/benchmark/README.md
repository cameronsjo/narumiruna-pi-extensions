...
Each treatment Pi process opens one persistent MCP stdio session and verifies that its child PID is committed to that daemon before measured calls begin.
...
This one-shot CLI setup is unmeasured and is not accepted as treatment transport evidence.
...
This study measures the package schemas, skill, model tool-call turn, persistent MCP stdio bridge, and tool-result envelope around the same evidence text.
A treatment result succeeds only when its non-model details report verified daemon-backed `mcp-stdio` transport.
...
The treatment extension keeps one MCP stdio child for that Pi session and closes it when the trial RPC process shuts down.
Persistence therefore spans calls within one trial, not separate trial processes.
Every successful measured pi-cbmem result must carry `transport: "mcp-stdio"` and `daemonBacked: true`; the scorer rejects missing or one-shot evidence.
...
It stores hashes, byte counts, exact grader values, tool names, tool arguments, verified measured-transport details, usage, timing, and package provenance.
...
- A failed treatment MCP session has no one-shot CLI fallback, so the affected trial fails instead of silently changing the measured transport.
