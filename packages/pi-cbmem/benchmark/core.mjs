...
export const BENCHMARK_ID = "pi-cbmem-retrieval-comparison:v3";
...
export function scoreTrial({
	...
	const unverifiedCbmemResults = successfulCbmemResults.filter(
		(result) => result.bridge?.transport !== "mcp-stdio" || result.bridge?.daemonBacked !== true,
	);
	...
	if (arm === "cbmem" && unverifiedCbmemResults.length > 0) {
		errors.push("measured cbmem result lacked verified daemon-backed mcp-stdio evidence");
	}
