...
async function executeTrial({ arm, evidencePacket, processStarted, project, rpc, task }) {
	...
	return {
		...
		method: {
			...
			measuredTransport: captured.toolResults
				.filter((result) => CBMEM_TOOLS.includes(result.name))
				.map((result) => ({ name: result.name, bridge: result.bridge })),
			toolCalls: captured.toolCalls,
	...
	onEvent(event) {
		...
		if (event.type === "tool_execution_start") {
			...
			this.toolResults.push({
				...
				text,
				bridge: bridgeEvidence(event.result),
			});
			this.evidenceText += `\n${text}`;
			if (
				this.timeToEvidenceCompleteMs === undefined &&
...
function bridgeEvidence(result) {
	const bridge = result?.details?.bridge;
	if (bridge?.transport !== "mcp-stdio" || bridge?.daemonBacked !== true) return undefined;
	return {
		transport: bridge.transport,
		daemonBacked: bridge.daemonBacked,
		...(Number.isSafeInteger(bridge.serverPid) ? { serverPid: bridge.serverPid } : {}),
	};
}
