...
Use `subagent_spawn`, `subagent_inspect`, `subagent_cancel`, `subagent_wait`, and the shared bidirectional `subagent_send` contract directly or through a user-designed skill after upgrading, and start a fresh Pi session so stored calls do not request removed tool names or the legacy `subagent_send` schema.
